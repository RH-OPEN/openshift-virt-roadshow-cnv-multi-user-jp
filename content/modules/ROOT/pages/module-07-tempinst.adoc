...
. インストールウィザードの自動応答のため、*Sysprep* アクションを定義します。画面右上の *+* マークをクリックして *Import YAML* 画面を表示し、以下のYAML定義を貼り付けて、 *Create* をクリックします。
+
image::module-07-tempinst/16a_create_configmap.png[link=self, window=blank, width=100%]
+
...
.. Disk size *5GiB* に縮小します。
.. *Disk source* のサイズはデフォルト値 *60 GiB* を設定したままにします。
...
. *Customize and create VirtualMachine* 画面で、*Boot mode* オプションの横にある編集ボタン（鉛筆マーク）をクリックします。
+
...
. *YAML* タブを選択し、YAML定義の以下のフィールドに、次の値を追加します。
+
IMPORTANT: スクリーンショットのように、インデントを正確に記載してください。

.. *disks* フィールド( *spec*.*template*.*spec*.*domain*.*devices*.*disks* )
+
```
- cdrom:
    bus: sata
...
```
+
image::module-07-tempinst/20a_sysprep_disks.png[link=self, window=blank, width=100%]
+
.. *volumes* フィールド（ *spec*.*template*.*spec*.*volumes* ）
+
```
- name: sysprep
  sysprep:
...
```
+
image::module-07-tempinst/20b_sysprep_volumes.png[link=self, window=blank, width=100%]
+
...
+
. 仮想マシンは、ISOイメージのダウンロード、設定、インスタンスの起動により、プロビジョニングプロセスを開始します。
+
