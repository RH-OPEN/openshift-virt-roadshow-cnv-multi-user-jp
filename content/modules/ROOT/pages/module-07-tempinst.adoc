...
. 左側のメニューから、*Workloads* -> *ConfigMap* に移動し、名前が *move-webapp-vmware-vm* から始まるConfigMapが存在するか確認します。存在する場合、対象のConfigMapの3点メニューをクリックし、表示されたメニューより *Delete ConfigMap* を選択してConfigMapを削除します。
対象のConfigMap全てに対し削除を実施し、プロジェクト内に名前が  *move-webapp-vmware-vm* から始まるConfigMapが存在しない状態となったら次に進んで下さい。
