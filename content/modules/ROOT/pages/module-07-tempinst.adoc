...
. 表示されている *windows* PVCを見つけ、右側の3点メニューから *Clone PVC* を選択します。ポップアップするメニューで、新しいPVCに *windows-2k19-sysprep-template* という名前を付け、*アクセスモード* を *Shared access(RWX)* に設定し、*Clone* ボタンをクリックします。
+
image::module-07-tempinst/29_Clone_Menu.png[link=self, window=blank, width=100%]

. 一度これを保存しておけば、今後Windows VMを素早く作成するのに使うことができる。

. *仮想化* -> *カタログ* メニューに戻り、*ディスク・ソース* として *PVC(clone PVC)* のオプションを選択し、クローンする *PVC name* として *Windows-2k19-Sysprep-Template* PVCを選択し、*Quick create VirtualMachine* ボタンをクリックして、新しい仮想マシンをクイック作成するためのブート・ソースとしてこのクローンPVCを使用します。
+
