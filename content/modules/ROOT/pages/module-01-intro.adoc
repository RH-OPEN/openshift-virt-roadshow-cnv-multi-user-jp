...
リマインダーとして、OpenShift コンソールには以下の情報でログイン可能です：

OpenShift コンソールは {console_url}[こちら^] からアクセスします。
...
. 表示された *VirtualMachines* ダッシュボードを調べます。現在、いくつかの VM がリストされていますが、これらは起動していません：
+
...
. *Overview* タブに移動し、ワーカーノードが稼働しているノードを確認します：
+
